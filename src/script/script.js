class PageBuilder {
    ...
    createBlock_filter(data) {
        let block = document.createElement("button"),
            ...
            ulContainer = document.createElement("div"),
        ...
        ulContainer.classList.add("filterContainer");
        ulContainer.appendChild(ul);

        block.appendChild(span);
        block.appendChild(ulContainer);
    ...
    createBlock_grid(data) {
        let block = document.createElement("ol"),
            ...
            showEmpty = function(name){
                ...
                title.innerText = ":(";
                descr.innerText = `No projects in '${name || "unknown"}' category`;
                empty.appendChild(document.createElement("div"));
                empty.firstElementChild.appendChild(title);
                empty.firstElementChild.appendChild(descr);
        ...
        if(visibleCount == 0) showEmpty(filter);
    ...
    createBlock_svg_link(data, className) {
        ...
        if (data.value["stroke_width"])
            path.setAttribute("stroke-width", data.value["stroke_width"]);
        else
        ...
        if (data.value.url)
            block.addEventListener("click", () => window.open(data.value.url));
    ...
    compositePopUpBody(data) {
        ...
        out.firstElementChild.classList.add("noselect");
        ...
        out.children[1].classList.add("noselect");
    ...
    compositeMenu(menu) {
        ...
        for (let i of menu) {
            let li = document.createElement("a"),
                clickFunction = (e) => {
                    this.rebuild(i.page);
                    popUpMenu.firstElementChild.querySelector(".current").classList.remove("current");
                    e.currentTarget.classList.add("current");
                    e.preventDefault();
                };
            li.setAttribute("value", i.page);
            li.innerText = i.name;//`<a href="/">${i.name}</a>`;
            li.href = i.href;
            ...
            clone.addEventListener("click", clickFunction);
            out.appendChild(li);
